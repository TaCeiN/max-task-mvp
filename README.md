# UniTask Tracker (VK Mini App stub)

Моно-репозиторий: FastAPI (backend) + React/Vite (frontend). Хранилище — SQLite. RU интерфейс, тёмная/светлая темы, задачи, заметки, теги, режимы фокуса (Помодоро), напоминания по дедлайнам.

## Быстрый старт

### Установка и запуск (без Docker)

**📖 Подробная инструкция:** [LOCAL_SETUP.md](LOCAL_SETUP.md)

Это самый простой способ запустить проект на новом ПК. Инструкция включает:
- Установку всех зависимостей
- Настройку backend, webhook сервера и frontend
- Решение типичных проблем

### Краткая инструкция

#### Backend

**Windows (PowerShell):**
```powershell
cd backend
python -m venv .venv
.venv\Scripts\Activate.ps1
pip install -r requirements.txt
uvicorn app.main:app --reload --host 0.0.0.0 --port 8000
```

**Mac/Linux:**
```bash
cd backend
python3 -m venv .venv
source .venv/bin/activate
pip install -r requirements.txt
uvicorn app.main:app --reload --host 0.0.0.0 --port 8000
```

#### Frontend
```bash
cd frontend
npm install
# Создайте файл .env.local с содержимым: VITE_API_URL=http://localhost:8000
npm run dev
```

После запуска:
- **Фронтенд**: http://localhost:5173
- **Бэкенд**: http://localhost:8000

### Авторизация

Приложение поддерживает автоматическую авторизацию через URL параметры:

```
http://localhost:5173/login?user_id=5107783&first_name=Иван&last_name=Иванов&username=ivan_ivanov
```

Параметры:
- `user_id` (обязательный) - уникальный идентификатор пользователя
- `first_name` (опционально) - имя пользователя
- `last_name` (опционально) - фамилия пользователя
- `username` (опционально) - имя пользователя (username)

Приложение автоматически извлечёт данные из URL, выполнит авторизацию/регистрацию и перенаправит на главную страницу.

## Деплой

Проект поддерживает два варианта деплоя через Docker. Подробная инструкция: [DOCKER.md](DOCKER.md)

### Вариант 1: Деплой на GitHub Pages (автоматический)

Автоматическая сборка фронтенда и деплой на GitHub Pages + запуск backend и webhook серверов.

**Быстрый старт:**

1. **Создайте файл `.env` в корне проекта:**
   ```bash
   # Скопируйте пример
   copy env.example .env
   # Или на Linux/Mac: cp env.example .env
   # Отредактируйте .env и заполните все переменные
   ```

2. **Заполните обязательные переменные в `.env`:**
   - `MAX_BOT_TOKEN` - токен бота (получите от MasterBot)
   - `BACKEND_URL` - адрес backend API
   - `WEBHOOK_URL` - адрес webhook сервера
   - `SECRET_KEY` - секретный ключ для JWT (сгенерируйте как описано в DOCKER.md)
   - `GITHUB_TOKEN` - токен для деплоя на GitHub Pages
   - `GITHUB_REPO` - репозиторий GitHub Pages (например: tacein/tacein.github.io)
   - `GITHUB_USER` - пользователь GitHub
   - `GITHUB_EMAIL` - email для коммитов

3. **Запустите деплой:**
   ```bash
   # Windows
   deploy-github.bat
   
   # Linux/Mac
   chmod +x deploy-github.sh
   ./deploy-github.sh
   ```

Что происходит:
- ✅ Собирается фронтенд с адресами из `.env`
- ✅ Фронтенд автоматически деплоится на GitHub Pages
- ✅ Запускаются backend и webhook серверы
- ✅ Автоматически подписывается на webhooks

### Вариант 2: Ручной деплой фронтенда

Сборка фронтенда + запуск backend и webhook серверов. Фронтенд собирается в Docker volume для последующего ручного деплоя.

**Быстрый старт:**

1. **Создайте файл `.env` в корне проекта:**
   ```bash
   # Скопируйте пример
   copy env.example .env
   # Или на Linux/Mac: cp env.example .env
   # Отредактируйте .env и заполните все переменные (без GITHUB_TOKEN)
   ```

2. **Заполните обязательные переменные в `.env`:**
   - `MAX_BOT_TOKEN` - токен бота
   - `BACKEND_URL` - адрес backend API
   - `WEBHOOK_URL` - адрес webhook сервера
   - `SECRET_KEY` - секретный ключ для JWT

3. **Запустите сборку:**
   ```bash
   # Windows
   deploy-manual.bat
   
   # Linux/Mac
   chmod +x deploy-manual.sh
   ./deploy-manual.sh
   ```

4. **Скопируйте файлы из volume:**
   ```bash
   # Linux/Mac
   docker run --rm -v unitask_frontend-build:/source -v $(pwd)/frontend-dist:/dest alpine sh -c 'cp -r /source/* /dest/'
   
   # Windows
   docker run --rm -v unitask_frontend-build:/source -v %cd%\frontend-dist:/dest alpine sh -c "cp -r /source/* /dest/"
   ```

Что происходит:
- ✅ Собирается фронтенд с адресами из `.env`
- ✅ Собранные файлы сохраняются в Docker volume `frontend-build`
- ✅ Запускаются backend и webhook серверы
- ✅ Автоматически подписывается на webhooks
- ✅ Вы копируете файлы из volume для ручного деплоя

### Генерация SECRET_KEY

JWT ключ должен быть случайной строкой минимум 32 символа:

**Python:**
```python
import secrets
print(secrets.token_urlsafe(32))
```

**OpenSSL:**
```bash
openssl rand -hex 32
```

**PowerShell (Windows):**
```powershell
-join ((48..57) + (65..90) + (97..122) | Get-Random -Count 32 | % {[char]$_})
```

Подробнее см. [DOCKER.md](DOCKER.md)

